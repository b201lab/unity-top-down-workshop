...
Sekadar informasi, selain keenam tatap muka utama tersebut juga ada tatap muka lain yang umumnya digunakan untuk keperluan yang lebih spesifik seperti Sprite Editor, Animation Editor, Audio Mixer, dan sebagainya.
...
- Pembuatan _scene_ baru pada Unity dilakukan melalui tatap muka Hierarchy Window.
- Sama seperti pembuatan _scene_ baru, terdapat dua cara untuk menambahkan _game object_ baru, yakni dengan menekan tombol `+` pada bagian kiri atas lalu pilih jenis _game object_ yang diinginkan atau dengan klik kanan, lalu `Create`, dan pilih jenis _game object_ yang diinginkan.
- Sebagai contoh kita bisa memilih _game object_ dengan jenis `3D Object` -> `Sphere`. Maka akan muncul object lingkaran pada Scene View maupun Game View.
  > Walaupun yang dibuat adalah _game_ jenis 2D, Unity tetap memperlakukan _game_ tersebut sebagai game 3D dengan perspektif kamera 2D. Hal ini bisa dilihat dengan mematikan _toggle_ `2D` pada Scene View.
...
1. Mengubah langsung pada Scene View menggunakan _tools_ yang tersedia pada _toolbar_ (sebelah kiri atas), seperti Move Tool, Rotate Tool, Scale Tool, dan sebagainya.
...
Sehingga akan didapatkan objek lingkaran besar yang berada tepat di tengah-tengah _scene_.
...
- Pembuatan _component_ baru dengan C# Script dilakukan melalui tatap muka Project Window.
  > Untuk percobaan kali ini, kita akan membuat _component_ C# Script pada _folder_ `1-Pengenalan-Konsep/Scripts`. jadi pastikan Project Window sudah berada pada _folder_ tersebut.
- Terdapat dua cara untuk menambahkan _component_ C# Script, yakni dengan menekan tombol `+` pada bagian kiri atas lalu pilih `C# Script` atau dengan klik kanan, lalu `Create`, dan pilih `C# Script`.
...
- Buka C# Script yang telah dibuat dengan cara _double_ klik.
  > Pada tahap ini Unity akan membuka text editor, jika Visual Studio tidak terinstall, bisa menggunakan text editor apapun seperti Notepad.

### Memrogram Component Centerized

Secara _default_, isi dari C# Script yang baru dibuat adalah sebagai berikut:
...
Inti dari _component_ yang dibuat menggunakan C# Script ada pada fungsi `Start()` dan `Update()`.
Sesuai dengan penjelasan pada _comment_, fungsi `Start()` akan digunakan untuk melakukan eksekusi pertama di awal, sedangkan fungsi `Update()` akan digunakan untuk melakukan eksekusi yang dipanggil secara terus menerus di setiap _frame_.
Pada percobaan kali ini, kita akan fokus pada fungsi `Start()` saja karena proses untuk membuat _game object_ berada di tengah cukup dilakukan sekali.
...
`Vector3` sendiri merepresentasikan posisi 3D yang terdiri dari x, y, dan z, seperti pada nilai `Position` pada component `Transform` di Inspector Window.
...
- Setelah _component_ ditambahkan, klik tombol `Play` pada Toolbar.
  > Pada tahap ini, tampilan akan terfokuskan pada Game View, dan objek lingkaran akan secara otomatis terletak di tengah saat _game_ sedang berlangsung.
...
bagian selanjutnya akan membahas gerakan dan animasi dari suatu _game object_, serta mengatur interaksi antara pemain dengan game yang akan dibuat.
